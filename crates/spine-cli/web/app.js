...
function render(s){const raw=JSON.stringify(s);if(raw===previous)return;previous=raw;
 ...
 document.getElementById('notice').textContent=s.notice||'';document.getElementById('input').placeholder=s.busy?'Guide the active run at its next tool boundary':'Message Spine';document.getElementById('send').textContent=s.busy?'Guide':'Send';
